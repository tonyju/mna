...
def etime(e0 = None):
    ...
    if dt < 60:
        ...
        seconds = dt % 60
        minutes = dt // 60
        s1 = '%d m and %.5f s' % (minutes, seconds)
    elif dt >= 3600 and dt < 86400:
        hours = dt // 3600
        minutes = (dt - hours) // 60
        seconds = (dt - hours) % 60
        s1 = '%d h, %d m and %.5f s' % (hours, minutes, seconds)
    else:
        days = dt // 86400
        hours = (dt - days) // 3600
        minutes = (dt - days - hours) // 60
        seconds = (dt - days - hours) % 60
        s1 = '%d d, %d h, %d m and %.5f s' % (days,
         hours,
         minutes,
         seconds)
    return (e1, s1)
...
for iter in range(8):
    ...
    for j in range(len(powsys.onephasepqbrancharray)):
        ...
        if not t.frnode==-1:
            x1=dx[t.frnode]
            x2=dx[t.frnode+nnode]
        if not t.tonode==-1:
            x3=dx[t.tonode]
            x4=dx[t.tonode+nnode]
        x5=dx[t.branchid+2*nnode]
        x6=dx[t.branchid+2*nnode+n2branch]
        #print '???',t.branchid,2*nnode,t.branchid+2*nnode+n2branch,x5,x6
        #fre
        if not t.frnode==-1:
            frerow.append(t.branchid)
            frecol.append(t.frnode)
        ...
        if not t.tonode==-1:
            frerow.append(t.branchid)
            frecol.append(t.tonode+nnode)
            fredata.append(-x6)
            
        frerow.append(t.branchid)
        frecol.append(t.branchid+2*nnode)
        fredata.append(x1-x3)
        
        frerow.append(t.branchid)
        frecol.append(t.branchid+n2branch+2*nnode)
        fredata.append(x2-x4)
        #fim
        if not t.frnode==-1:       
            fimrow.append(t.branchid)
            fimcol.append(t.frnode)
            fimdata.append(-x6)
        if not t.frnode==-1:
            fimrow.append(t.branchid)
            fimcol.append(t.frnode+nnode)
        ...
        fimrow.append(t.branchid)
        fimcol.append(t.branchid+2*nnode)
        fimdata.append(x2-x4)
        
        fimrow.append(t.branchid)
        fimcol.append(t.branchid+n2branch+2*nnode)
        fimdata.append(-x1+x3)  
              
        rval[2*nnode+t.branchid]=-np.real(((x1-x3)+1j*(x2-x4))*(x5-1j*x6))+t.p
        rval[2*nnode+n2branch+t.branchid]=-np.imag(((x1-x3)+1j*(x2-x4))*(x5-1j*x6))+t.q      
        #print '""":::...',t.p,x1,x2,x3,x4,x5,x6,re(((x1-x3)+I*(x2-x4))*(x5-I*x6))
        #print t.p,rval[2*nnode+t.branchid],((x1-x3)+1j*(x2-x4))*(x5-1j*x6)
        #print t.q,rval[2*nnode+n2branch+t.branchid],((x1-x3)+1j*(x2-x4))*(x5-1j*x6)
        #print t.branchid
    ...
    #e0, s0 = etime()
    #e_begin = e0
    ddx=linalg.spsolve(J, F)
    #e0, s0 = etime(e0)
    #print s0,'wholematrix','s'
    #print 'F',F
    dx=dx+ddx
    print 'iter',iter#,rval[0]
    #print 'ddx',np.max(np.abs(ddx))
    tolnow=np.max(np.abs(ddx))
    #print F

    #A=J11  #inverse
    #print J11
    Ainv=scipy.sparse.linalg.inv(J11)
    #print Ainv
    B=A2diag
    FF=scipy.sparse.vstack([fre,fim],format="csr")
    C=FF[:,0:2*nnode]
    D=FF[:,2*nnode:2*nnode+2*n2branch]
    #print 'C',C
    #print 'D',D
    b2=F[2*nnode:2*nnode+2*n2branch]
    E=D-C*Ainv*B
    e0, s0 = etime()
    e_begin = e0
    ddy=linalg.spsolve(E, b2)
    e0, s0 = etime(e0)
    print s0,'partmatrix','s'    
    #print 'ddy',ddy
    ddx1=-Ainv*B*ddy
    #print 'ddx1',ddx1
    #ddx=scipy.sparse.vstack([ddx1,ddy],format="csr")
    #for iddx1 in range(len()):
        #ddx=ddx1+
print rval0
