...
i=0
with open('seregulator.csv', 'rb') as f:
    reader = csv.reader(f, delimiter=',', quoting=csv.QUOTE_NONE)
    #print len(reader)
    for row in reader:
        if i==0:
        ...
        #print row[0],row[1],row[2],row[3],row[4],len(row)      
i=0
with open('sevoltagesource.csv', 'rb') as f:
    reader = csv.reader(f, delimiter=',', quoting=csv.QUOTE_NONE)
    #print len(reader)
    for row in reader:
        if i==0:
...
i=0
with open('seacline.csv', 'rb') as f:
    reader = csv.reader(f, delimiter=',', quoting=csv.QUOTE_NONE)
    #print len(reader)
    for row in reader:
        if i==0:
...
#print nodearray
i=0
with open('setrfmwinding.csv', 'rb') as f:
    reader = csv.reader(f, delimiter=',', quoting=csv.QUOTE_NONE)
    #print len(reader)
    for row in reader:
        if i==0:
...
for i in range(0,nnode):
    ...
    vnimrow.append(i)
    vnimcol.append(0)
    vnredata.append(dx[i])
    vnimdata.append(dx[i+nnode])
    
    
row = np.array(vnrerow)
col = np.array(vnrecol)
data = np.array(vnredata)
vnre=csr_matrix((data, (row, col)), shape=(nnode,1))
row = np.array(vnimrow)
col = np.array(vnimcol)
data = np.array(vnimdata)
vnim=csr_matrix((data, (row, col)), shape=(nnode,1))

#get stamat
vb1re=A1T*vnre
vb1im=A1T*vnim

ib1re=G*vb1re-B*vb1im
print 'ib1re',ib1re
...
print 'i1amag',np.abs((dx[2*nnode]+1j*dx[2*nnode+n2branch]))
print 'i1bmag',np.abs(dx[2*nnode+1]+1j*dx[2*nnode+n2branch+1])
print 'i1cmag',np.abs(dx[2*nnode+2]+1j*dx[2*nnode+n2branch+2])
